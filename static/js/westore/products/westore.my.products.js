...
WestoreMyProducts.isPInt = function(v) {
	var reg = new RegExp("^[1-9][0-9]*$");
 	return reg.test(v);
};

WestoreMyProducts.init = function(){
	//先将所有菜单收起来
	$("dt").removeClass("open");
	//再将在线商品一栏打开并且选中功能
	WestoreMyProducts.bind();
};

WestoreMyProducts.showQueryResult = function(data){
...
//获取当前文件名
WestoreMyProducts.QueryFileName = function(){
...
//根据筛选条件查找商品
WestoreMyProducts.QueryProducts = function(index){
	//商家名称
	...
	//传递参数ajax（别忘了把页码传过去）
	var postData = "productsName=" + productsName
	...
	//调用ajax
	$.ajax({
		url:"/WeStoreOperation/index.php/westore/queryMyProducts",
		type:"POST",
		data:postData,
		dataType:"json",
		error:function(){
		...
		success:function(data){
			...
			if(data.totalNum > 0){
				...
				$("[name='pageno']").val("");
				WestoreMyProducts.showQueryResult(data);	
...
WestoreMyProducts.bindButtons = function(){
	//查询按钮
	$("[name='productsQuery']").on('click',function(){
		WestoreMyProducts.QueryProducts(1);
	});
...
//分页函数
WestoreMyProducts.bindPage = function(classname, callback) {
	...
	$(parent_div + " a.page-first").on('click', function() {
		...
		if (curpage != 1 &&  WestoreMyProducts.isPInt(curpage)) {
			callback(1);
		}
	});
	$(parent_div + "a.page-last").on('click', function() {
		...
		if (total_page != curpage &&  WestoreOrderQuery.isPInt(curpage) && WestoreMyProducts.isPInt(total_page)) {
			callback(total_page);
		}
	});
	$(parent_div + "a.page-next").on('click', function() {
		...
		if (total_page != curpage && WestoreMyProducts.isPInt(curpage) && WestoreMyProducts.isPInt(total_page)) {
			callback(curpage + 1);
		}
	});
	...
	$(parent_div + "a.page-go").on('click', function() {
		...
		var total_page = parseInt($(parent_div + "label[name=total_page]").text(), 10);
		if ( WestoreOrderQuery.isPInt(curpage) && WestoreMyProducts.isPInt(total_page) && WestoreMyProducts.isPInt(go_page) && go_page <= total_page && go_page != curpage) {
			callback(go_page);
		}
	});
};

WestoreMyProducts.Do_search = function(page){
	WestoreMyProducts.QueryProducts(page);
};


WestoreMyProducts.bindPageButton = function(){
	WestoreMyProducts.bindPage('products_page', WestoreMyProducts.Do_search);
};

WestoreMyProducts.bind = function(){
	//监听按钮
	this.bindButtons();
	this.bindPageButton();
};

WestoreMyProducts.init();
